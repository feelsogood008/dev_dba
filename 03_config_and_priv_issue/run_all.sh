...
$MYSQL_CMD < 01_create_test_db_and_user.sql 

echo "[2단계] 테이블, 뷰, 프로시저 생성 (관리자)"
$MYSQL_CMD < 02_create_objects_as_admin.sql

echo "[3단계] 현재 계정 권한 확인"
$MYSQL_CMD < 03_check_user_privileges.sql

echo "[4단계] svc_api 서비스 계정으로 접근 테스트, new_payments 테이블 접근 실패"
mysql -u svc_api -p'P@ssw0rd' demo_db < 04_access_test_as_user.sql

echo "[5단계] 권한 보완 및 재확인"
$MYSQL_CMD < 05_fix_privileges_and_config.sql
